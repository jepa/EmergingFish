...
TransIndex <- function(Centroids, # Expecting the results from the GetCentroids function
  ...
  historic_sd <- Trans_index %>% 
    ...
    group_by(taxon_key,neighbour_id,year) %>% #Average historic data per year first
...
GetTransIndex <- function(taxon_key,
  ...
  Step_one <- bind_rows(
    ...
    name <- paste(my_path("R",extra_path = "/Emergence_alt_sens/"),taxon_key,"_emergence.txt",sep="")
