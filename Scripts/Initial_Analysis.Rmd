...
# Time of Emergence of SST using methods here
Toe_sst <-fread(paste(my_path("D"),"Climate/ToE_sst.csv", sep ="")) %>% 
  clean_names()
...
Eez_biogeo <- EEZ_CellID %>% 
  ...
  group_by(eezid,basin,biome) %>% 
  summarise(nbiom=n(),
            nbasin=n()) %>% 
  # Select the more frequent biome per eez
  group_by(eezid,basin) %>% 
  top_n(1,nbiom) %>% 
...
for(i in 1:2){
  ...
  name_save <- paste("./Figures/emer_map_rev_",tresh_list[i],".png",sep = "")
...
M_all_var <- lm(emerging_yr ~ sst_toe + status + biome + dem_pel + DemersPelag, data = Lm_data)
...
# Model 2; non demPel
M2 <- lm(emerging_yr ~ sst_toe + status + biome + dem_pel, data = Lm_data)
...
# Model 3; non biome
M3 <- lm(emerging_yr ~ sst_toe + status + dem_pel + DemersPelag, data = Lm_data)
...
# Model 4; non status
M4 <- lm(emerging_yr ~ sst_toe + biome + dem_pel + DemersPelag, data = Lm_data)
...
# Model 5; non sst
M5 <- lm(emerging_yr ~ status + biome + dem_pel + DemersPelag, data = Lm_data)
...
# Model 6; all + basin
M6 <- lm(emerging_yr ~ sst_toe + status + biome + basin + dem_pel + DemersPelag, data = Lm_data)
...
M7 <- lm(emerging_yr ~ sst_toe + category + biome + dem_pel + DemersPelag, data = Lm_data)
...
M8 <- lm(emerging_yr ~ sst_toe + category + biome + dem_pel + DemersPelag + latitude, data = Lm_data)
...
tibble(
  "model" = c("all variables", "no demPel","no biome","no status","no sst","all + basin","Categories","lat centroids"),
