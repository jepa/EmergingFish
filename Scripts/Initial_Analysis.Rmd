...
# Taxon_list_emer <- list.files(my_path("R",extra_path = "Emergence_2005"))
Taxon_list_emer <- list.files("~/Desktop/Emergence_p95_alt")

# clean names for function
Taxon_list_emer <- gsub("_.*","",Taxon_list_emer) 


# Paths <- paste(my_path("R",extra_path = "Emergence_raw_p95"),Taxon_list_emer,"_emergence_raw.csv",sep="")[1:10]

Paths <- list.files("~/Desktop/Emergence_p95_alt",full.names = T)

# Load all sp in one table
Eme_Alt_Res <- bind_rows(lapply(Paths, fread)) %>% 
  left_join(Neighbours,
            # by = c("eez_name","eez_neighbour")
  ) %>%
  rename(sf_eez_name=eez_name) %>%
