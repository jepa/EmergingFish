...
```{r vulnerabillity_index, eval = T}

# Per EEZs
Eez_d_hist <- Eme_Res %>% 
  group_by(eezid ,sau_fishing_entity,fishing_entity_id,tresh) %>% 
...
Index_data <- Eez_d_hist %>% 
  ...
  mutate(
    index = (participation/value),
    index = ifelse(is.na(index),0,index)
  ) %>% 
  select(1:3,value,participation,index,everything()) 
...
Index_data <- Index_data %>% 
  mutate(
    index = 2000 + (index-min_index)*(2100-2000)/(max_index-min_index)
  )


Value_map_data <- Index_data %>% 
  select(sau_fishing_entity,fishing_entity_id,tresh,variable,participation = indexb)
