...
GetCentroids <- function(taxon_key,
  ...
  if(nr > 0){
    ...
    CentroidsFx <- function(Ids){  
      ...
      ### Code to produce graphs for Brazil and Uruguay
...
TransIndex <- function(Centroids, # Expecting the results from the GetCentroids function
  ...
  Trans_index <- Centroids_id %>% 
    ...
    # filter(year > 1960) %>% # drop first 10 years of running mean
  ...
  # Valid ensembles
  # Only perform analysis on ensembles containing all years between 1961 and 2040
  ...
  historic_sd <- Trans_index %>% 
    filter(year <= Hist_y) %>% 
    # Alternative method sensitivity analysis
    # group_by(taxon_key,neighbour_id,year) %>% #s.d. historic data per year first
  ...
  if(Plot == T & length(historic_sd$hist_mean) > 1){
    ...
    ggplot() +
      ...
      geom_line(data = Trans_index,
                aes(
                  x = year,
                  y = RMean,
                  color = as.factor(ensemble)
                ),
                alpha = 0.3
      ) +
      geom_vline(xintercept = Hist_y,colour="blue",alpha = 0.5) +
      scale_x_continuous(breaks = seq(1960,2100,5),limits = c(1960,2100)) +
      scale_color_brewer("Ensembles", palette = "Greens") +
      theme_classic()
  ...
  # If no historic data then emerging year is the first year of data
  if(nrow(historic_sd) == 0){
    ...
    Final_Result <- mean_sd_index %>%
      ...
      summarise(emerging_yr= min(year)) %>%
      # summarise(emerging_yr= max(year)) %>% # Last year that treshold is crossed
      left_join(EEZ_ids,
                by = "neighbour_id") %>%
      ungroup() %>%
...
GetTransIndex <- function(taxon_key,
  ...
  #   print(Ids[i])
  #   print(TransIndex(Step_one,Ids[i]))
  # }
  ...
  name <- paste(my_path("R",extra_path = "/Emergence_95/"),taxon_key,"_emergence.txt",sep="")
...
spp_done <- list.files(my_path("R", extra_path = "Emergence_alt_sens"))
...
This is a hypothetical figure to show the idea behind the ToE. It does not follow the actual method but only shows the trend. 

```{r tsi_figure, eval = F}

# Create dummy data
dummy_data <- data.frame(
  year = seq(1951,2100,1),
  tsi = c(runif(80,-10,10),
          runif(20,-10,4),
          runif(30,-12,0),
          runif(20,-14,-4)),
  tsia = c(runif(80,-10,10),
          runif(20,-10,4),
          runif(30,-12,0),
          runif(20,-14,-4)),
  tsib = c(runif(80,-10,10),
           runif(20,-10,4),
           runif(30,-12,0),
           runif(20,-14,-4)),
  tsic = c(runif(80,-10,10),
           runif(20,-10,4),
           runif(30,-12,0),
           runif(20,-14,-4))
) %>% 
    rowwise() %>% 
    mutate(
      mean_tsi = mean(c(tsi,tsia,tsib,tsic),na.rm=T)
    ) %>% 
    ungroup() %>% 
  group_by() %>% 
  mutate_all(funs(zoo::rollmean(., 
                          10, 
                          align = "left", 
                          fill = NA,
                          na.rm=T)
  )) %>% 
  gather("eme","value",tsi:tsic)


# Get min, max and toe for box/noise
min <- dummy_data %>% 
  filter(year < 2006) %>% 
  pull(value) %>% 
  min()
...
# ToE
toe <- dummy_data %>% 
  filter(year > 2005 & mean_tsi < min+0.5) %>% 
  slice(1)


# Make hypotetical figure
ggplot(dummy_data) +
  # Historical variation box (noise)
  geom_rect(
    aes(xmin=1951,
        xmax=2100,
        ymin=min+0.5,
  ...
  # Historical / Future division line
  geom_vline(xintercept = 2005, color = "#046C9A") +
  # Aesthetics
  scale_color_brewer(palette = "Oranges") +
  theme_classic()+
  theme(axis.text = element_blank(),
        axis.ticks = element_blank())+
  labs(x = "Transboundary shift index (TSI)",
       y = "Year")
    

# save image
ggsave(
  filename = "./Figures/tsi_fig.png",
  plot = last_plot(),
  width = 8,
  height = 4
)
...
This results only shows the average percentage change in proportion of all species within an EEZ without relationship with neighbours

### Functions and Data needed

#### Fun `SummaryProp`

This function estimates the total change in MCP proportion of each species for each EEZ. It looses the connection with neighbours as you only know for x species how much is the proportion changing.

```{r fun_SummaryProp, eval = F, echo =F}

# taxon_key = "600147"
# top_stocks_per_country
...
# Neighbor list and their respective id from Palacios-Abrantes et al 2020 (FishForVisa)
Neighbours <- my_path("D", extra_path = "Spatial/", name="Neighbours_eez_id.csv", read = TRUE)
...
# Write data file for Github

# toe_data <- bind_rows(lapply(Paths, fread)) %>% 
#   filter(
#     n_ens == 10
#   ) %>% 
#   left_join(Exploited_spp_list) %>% 
#             select(taxon_key,taxon_name,common_name,everything(),-genus,-species)
# 
# write_csv(toe_data, "./Data/toe_data.csv")
...
World_eez_sf <- st_read(my_path("G", extra_path = "Spatial/SAU/SAU_Shapefile", "SAUEEZ_July2015.shp")) %>% 
  rename(eez_name = Name) %>% 
  # st_transform(crs = 4326) %>% # 4326
...
# See chunk Emerging_control_panel
n_stock <- 9132
n_eez <- length(unique(Eez_centroid_sf$eez_name))
...
### Centroid Supplemental Figure
```{r}

centroid_map <- ggplot() +
  geom_sf(data = World_eez_sf, aes()) +
  geom_sf(data = subset(World_map, !sovereignt %in% c("Antarctica")), aes(),fill = "grey90") +
  geom_sf(data = Eez_centroid_sf, aes()) +
  my_ggtheme_m(leg_pos = "")

ggsave(
  plot = centroid_map,
  "./Figures/centroid_map.png",
  width = 8,
  height = 5,
  units = "in"
)
...
##### Fig. 2. Accumulative plots

Timeline of shifts in transboundary shared stock distribution. (A) Number of stocks (cumulative) whose distribution centroids that have shifted beyond natural variations over time (from 2006 to >2100), (B) Number of EEZs (cumulative) with the distribution centroid of at least one stock that have shifted beyond natural variations and (C) proportion of total shifting stocks and EEZs with at least one stock shifting beyond natural variability. Dashed lines represent the year by which countries have committed to reach full implementation of the 2030 Agenda, as well as mid and late change in range shift (Methods), respectively.
...
# How many between 2006 and 2030?
plot_data %>% 
  filter(taxon_emer <= 2030,
...
count_eez_plot <- ggplot() +
  ...
  my_ggtheme_p(leg_pos = "", hjust = 0.5) +
  theme(
...
percentage_plot <- ggplot() +
  ...
  my_ggtheme_p(leg_pos = "right", hjust = 0.5) +
  scale_fill_viridis(discrete = T,
                     begin = 0.1,
                     end = 0.7,
...
plot_all <- ggdraw() +
  # Count stock plot
  draw_plot(count_stock_plot, x = 0.01, y = 0, width = 0.29, height = 0.90) +
  # Count EEZ plot
  draw_plot(count_eez_plot, x = 0.31, y = 0, width = 0.29, height = 0.90) +
  # Percentage plot
  draw_plot(percentage_plot, x = 0.61, y = 0 , width = 0.29, height = 0.90) +
  # legend
  draw_plot(legend, x = 0.84, y = 0 , width = 0.20, height = 1) +
  # Labels
...
for(i in 1:2){
  ...
  no_toe_land <- World_map %>% 
    filter(name %in% Neighbours$eez_name, # Filter EEZs without Transboundary spp
           !name %in% World_map_part$name)%>% 
    sf::st_transform(crs = "+proj=eck4")  # Filter EEZs with ToE stocks
  ...
  map <- ggplot(toe_map_data) +
    ...
    # grey maps
    # ggplot() +
    geom_sf(data = grey_land, aes(), fill = "grey50", 
            # color =  "white",
            size = 0.1) +
    ...
    annotate("text",
             x = -14012580,
             y = -8357450,
             label = "Transboundary fish revenue (%)",
             color="black",
             size=5,
             angle=0,
             fontface="bold",
             hjust=0) +
    annotate("text",
             x = c(-3812580,-4500,3800000,7700000,11512580),
             y = -8400000,
    ...
    annotate("text",
             ...
             x = -13012580,
             y = -9657450,
             label = "Average year of range shift",
             color="black",
             size=5,
             angle=0,
             fontface="bold",
    ...
    annotate("text",
             # x = c(-79,-39,2,42,81),
             # y = -85,
             x = c(-4412580,-430000,3410000,7300000,11312580),
             y = -9657450,
  ...
  #   # Get the order of boxes
  #   filter(tresh == tresh_list[i],
  #          !is.na(un_sub_region)) %>%
  #   group_by(un_sub_region) %>%
  #   summarise(n_eezs = length(unique(sf_eez_name))) %>%
  ...
  #   )#,
  #   # For number labels
  #   # un_sub_region = paste(un_sub_region," (n = ",n_eezs,")",sep="")
  #   )
  ...
  boxplot_data <- Eme_Res %>%
    ...
    filter(tresh == tresh_list[i],
           !is.na(un_sub_region)) %>%
    group_by(un_sub_region) %>%
    summarise(order = mean(emerging_yr),
              n_eezs = length(unique(sf_eez_name))) %>%
    # Get the rest of the data
    left_join(Eme_Res) %>%
    filter(tresh == tresh_list[i],
           !is.na(un_sub_region)) %>%
  ...
  # ggsave("mean_boxplot.png",boxplot,width = 10)
  ...
  p <- ggdraw() +
    ...
    draw_plot(map, x = 0, y = 0.39, width = 1, height = 0.70) +
    # Regional boxplot
...
###### Lat Am Figure 3 for supplements
...
lat_am_map <- ggplot(toe_map_data) +
    geom_sf(
      aes(
        fill = reverse #original map
        # fill = value
    ...
    geom_sf(data = no_toe_eez, aes(), fill = "lightblue", color =  "white",size = 0.1) +
    geom_sf(data = no_toe_land, aes(), fill = "lightblue", color =  "white",size = 0.1) +
  coord_sf(xlim = c(-9000000,-5000000),ylim = c(1000000, 4000000)) + 
  facet_wrap(~tresh, ncol = 1) +
  scale_colour_viridis("",
                         limits = c(2000,2100),
                         breaks = seq(2000,2100,25),
                         # labels = c("","","","",""),
    ...
    my_ggtheme_m(map_type = "regional",leg_pos = "")

ggsave("Figures/lat_am_map.png",
       lat_am_map,
       width = 6,
       height = 8)
...
###### SP Islands Figure 3 for supplements

```{r}


# Value Map
spi_map_part <- World_map_nemer %>% 
  filter(
    # tresh ==  tresh_list[i],
    variable == "value"
  ) %>% 
  sf::st_transform(crs = 3832) 


# ToE Map 
spi_toe_map_data <- World_eez_sf %>% 
  left_join(Eez_d_hist,
            by = "eezid") %>% 
  filter(#tresh == tresh_list[i],
    metric == "mean_yr")%>% 
...
# Map with countries without ToE
spi_no_toe_eez <- World_eez_sf %>% 
  filter(eez_name %in% Neighbours$eez_name, # Filter EEZs without Transboundary spp
         !eez_name %in% toe_map_data$eez_name) %>%  # Filter EEZs with ToE stocks
  sf::st_transform(crs = 3832) 

spi_no_toe_land <- World_map %>% 
  filter(name %in% Neighbours$eez_name, # Filter EEZs without Transboundary spp
         !name %in% World_map_part$name)%>% 
  sf::st_transform(crs = 3832) # Filter EEZs with ToE stocks



spi_map <-  ggplot(spi_toe_map_data) +
  geom_sf(
    aes(
      fill = reverse #original map
      # fill = value
  ...
  # grey maps
  geom_sf(data = spi_no_toe_eez, aes(), fill = "lightblue", color =  "white",size = 0.1) +
  geom_sf(data = spi_no_toe_land, aes(), fill = "lightblue", color =  "white",size = 0.1) +
  coord_sf(xlim = c(9500000,1000000),ylim = c(-3500000, 1900000)) + 
  facet_wrap(~tresh, ncol = 1) +
  scale_colour_viridis("",
                       limits = c(2000,2100),
                       breaks = seq(2000,2100,25),
  ...
  my_ggtheme_m(map_type = "regional",leg_pos = "bottom") +
  theme(legend.key.width = unit(3, "line"))


ggsave("Figures/spi_map_bf.png",
       spi_map,
       width = 6,
       height = 8)
```
...
###### Splitted Figure 3 for supplements

Note this chunk needs `emergence_map_plot` to be ran.

Year of change in the shared distribution of 4,119 transboundary stocks and their contribution to fishing revenue. A) Contribution of shifting stocks to a country or territory's total fishing revenue from transboundary stocks. B) Average year of range shift across an EEZ’s transboundary stocks. EEZs with no distributional shift between 2006 and 2100 are represented in pale blue. This figure is a detached version of Figure 3


```{r supplement_figure_splitted}
...
# EEZ map
eez_map <- ggplot(toe_map_data) +
  geom_sf(aes(fill = value), # for 2000 to 2100),
          color =  "white",
          size = 0.1
  ) +
  geom_sf(data = grey_land, aes(),
          fill = "grey50", 
          color =  "white",
          size = 0.1) +
  geom_sf(data = no_toe_eez, aes(fill = as.numeric(value)),
          color =  "white",
          size = 0.1) +
  scale_fill_viridis("Average year of range shift",
                     limits = c(2000,2100),
                     breaks = seq(2000,2100,25),
                     labels = seq(2000,2100,25),
                     direction = -1,
                     na.value = "lightblue"
  ) +
  my_ggtheme_m(leg_pos = "bottom") +
...
land_map <- ggplot() +
  geom_sf(data = World_map_part,
          aes(fill = participation-2000, 
              color = participation-2000),
          size = 0.1
  )  +
  geom_sf(data = no_toe_land, aes(fill = as.numeric(value),
                                  color = as.numeric(value)),
          size = 0.1) +
  scale_fill_viridis("Transboundary fish revenue (%)",
                     limits = c(0,100),
                     breaks = seq(0,100,25),
                     labels = seq(0,100,25),
                     direction = 1,
  ...
  scale_color_viridis("Transboundary fish revenue (%)",
                      limits = c(0,100),
                      breaks = seq(0,100,25),
                      labels = seq(0,100,25),
                      direction = 1,
...
p <- ggdraw() +
  draw_plot(eez_map, x = 0, y = 0, width = 1, height = 0.5) +
  # Regional boxplot
  draw_plot(land_map, x = 0, y = 0.5, width = 1, height = 0.5) +
  # Labels
  draw_plot_label(label = c("A", "B"), 
                  size = 20,
                  x = c(0, 0), 
...
##### Fig. 4 Mapping the time of emergence of Top stocks

```{r emergence_map, eval = F}

top_stocks_per_country <- Eme_Res %>% 
  rename(geo_region = un_sub_region) %>% 
  group_by(sau_fishing_entity,fishing_entity_id,geo_region,tresh,taxon_key) %>% 
  summarise(n()) %>% 
  left_join(Sau_data,
            by = c("fishing_entity_id","taxon_key")) %>% 
  filter(!is.na(variable)) %>% 
  group_by(sau_fishing_entity,geo_region,tresh,variable) %>% 
  # Select top 5 most valuable stocks
  top_n(5,value)
...
top_toe <- Eme_Res %>% 
  rename(geo_region = un_sub_region) %>% 
  group_by(eezid,sau_fishing_entity,geo_region,tresh,taxon_key) %>% 
  summarise(mean_yr = mean(emerging_yr,na.rm=T)) %>% 
  right_join(top_stocks_per_country,
             by = c("sau_fishing_entity","geo_region","tresh","taxon_key")
  ) %>% 
  group_by(sau_fishing_entity,geo_region,tresh,variable) %>% 
  summarise_at(vars(mean_yr),
               c("mean"=mean,
                 "sd" = sd)) %>% 
  mutate(
    geo_region = ifelse(is.na(geo_region),"Other",geo_region)
  )
...
Eme_Res %>% 
  ...
  # View()
  summarise(
    n_countries = length(unique(sau_fishing_entity))
  ) %>% 
  mutate(per = n_countries/length(unique(Sau_data$fishing_entity_id))*100)

# Number of shifting stocks per geo-region
Eme_Res %>% 
  rename(geo_region = un_sub_region) %>% 
  group_by(eezid,sau_fishing_entity,geo_region,tresh,taxon_key) %>% 
  summarise(mean_yr = mean(emerging_yr,na.rm=T)) %>% 
...
###### The Map option with tops

```{r the map}

# Create data
Top_map_data <- top_toe %>% 
  filter(tresh == selected_tresh,
         variable == "value") # tried with catch but is quite the same


World_map_top <- World_eez_sf %>% 
  left_join(Matching_names) %>% 
  full_join(Top_map_data,
            by = c("sau_fishing_entity")
  ) %>% 
  filter(!is.na(variable)) %>% 
  sf::st_transform(crs = "+proj=eck4")
...
no_toe_eez <- World_eez_sf %>% 
  sf::st_transform(crs = "+proj=eck4") %>% 
  filter(eez_name %in% Neighbours$eez_name, # Filter EEZs without Transboundary spp
         !eez_name %in% World_map_top$eez_name) %>%  # Filter EEZs with ToE stocks
  rbind(russia_churchil)%>% 
  sf::st_transform(crs = "+proj=eck4")

no_toe_land <- World_map %>% 
  filter(name %in% Neighbours$eez_name, # Filter EEZs without Transboundary spp
         !name %in% World_map_top$name
  ) %>% 
  sf::st_transform(crs = "+proj=eck4")


map <- ggplot(World_map_top) +
  geom_sf(
    aes(
      fill = mean #original map
      # fill = value
    ),
    color =  "white",
    size = 0.1
  ...
  # grey maps
  # ggplot() +
  geom_sf(data = subset(World_map, sovereignt != "Antarctica"), aes(), fill = "grey50", color =  "white",size = 0.1) +
  geom_sf(data = no_toe_eez, aes(), fill = "lightblue", color =  "white",size = 0.1) +
  # geom_sf(data = no_toe_land, aes(), fill = "lightblue", color =  "white",size = 0.1) +
  # Set Participation labels
  scale_fill_viridis("Average year of range shift of top valued transboundary stocks",
                     limits = c(2000,2100),
                     breaks = seq(2000,2100,25),
                     direction = -1
  ) +
...
Supplemental box picture by habitat preference
...
# ------------------- #
# Test normal assumptions 
# ------------------- #

hist(Stats_data$emerging_yr) # Positively skewed
qqnorm(Stats_data$emerging_yr) # not normally distributed
skewness(Stats_data$emerging_yr) #0.61
kurtosis(Stats_data$emerging_yr) #2.11
...
# Kruskal-Wallis chi-squared = 242.11, df = 93, p-value = 3.128e-15
...
# Multiple differences but Eastern Asia, Lat. Ame. Car. and Polynesia are the ones who differ the most (e.g. differ with more regions).
...
##  Stock Share Ratio Analysis

### Fig. 5 and S5 Proportion + Netowrk

Note that this map is based on the data viz site: https://www.data-to-viz.com/story/MapConnection.html

#### Prepare data

```{r ssr_map_data, eval = T, echo = T}
...
# SELECT A TIME STEP
time_toplot <- "per_mid"
# time_toplot <- "per_early"
```

#### Get data ready for time step
...
# name <- paste0("./Figures/ssr_",time_toplot,".png")
# Save figure
# ggsave(plot =  p,
#        name,
#        width = 8,
...
#### Regional maps for supplements

Note that the following chunk needs `ssr_map_data` to work. 
...
lat_am_map <- ggplot() +
  # PLot EEZs
  geom_sf(data = subset(World_eez_sf,
                        !str_detect(eez_name,"Pacific") &
                        !str_detect(eez_name,"El Salvador") &
                        !eez_name %in% c("USA (East Coast)","Suriname","Guyana","French Guiana","Bermuda (UK)","Brazil")
  ), size = 0.1, fill = "white", col = "gray30") +
  # Plot Land information
  geom_sf(data = subset(n_taxa_map, sovereignt != "Antarctica"),
          aes(fill = avrg_spp_per),
          size = 0.1,
          col = "white") +
  geom_sf(data = subset(sf_ready_plot, percentage >= 0 & 
                          !str_detect(source,"Pacific") & 
                        !source %in% c("USA (East Coast)","Suriname","Guyana","French Guiana","Bermuda (UK)","Brazil","El Salvador") &
                        !str_detect(objective,"Pacific") &
                        !objective %in% c("USA (East Coast)","Suriname","Guyana","French Guiana","Bermuda (UK)","Brazil","El Salvador")
                        ), aes(color = percentage), size = 0.1, fill = "white") +
  geom_sf(data = subset(point_data,
                        !str_detect(source,"Pacific") & 
                        !source %in% c("USA (East Coast)","Suriname","Guyana","French Guiana","Bermuda (UK)","Brazil","El Salvador") &
                        !str_detect(target,"Pacific") &
                        !target %in% c("USA (East Coast)","Suriname","Guyana","French Guiana","Bermuda (UK)","Brazil","El Salvador")
                        ),
          aes(), color = "grey90") +
  geom_path(data = subset(arrows_data, percentage >= 0 &
                            !str_detect(objective,"Pacific") &
  ...
  my_ggtheme_m(map_type = "NA",leg_pos = "bottom") +
  theme(
...
name <- paste0("Figures/lat_am_",time_toplot,".png")
...
# Name to save
name_eu <- paste0("Figures/EU_",time_toplot,".png")
...
EU_map <- ggplot() +
  ...
  geom_path(data = subset(arrows_data, percentage >= 0),
            ...
            arrow = arrow(length = unit(0.03, "npc"),
                          ends = "first"),
            show.legend = NA) +
  coord_sf(xlim = c(-1300000,2000000),ylim = c(5500000, 7500000))  +
  labs(x = "", y = "") +
  scale_x_continuous(breaks = seq(-100,60,10)) +
  # scale_y_continuous(breaks = seq(10,30,10)) +
  my_ggtheme_m(map_type = "NA",leg_pos = "bottom") +
...
top_taxon_list <-  unique(top_stocks_per_country$taxon_key)
# Read proportion results
suppressMessages(
  top_prop_results <- PropChange(top_taxon_list, links="y",top = T) %>%
    filter(!is.na(eez_neighbour)) %>%
    mutate(per_mid_plot =ifelse(per_mid >= 100, 100,
                                ifelse(per_mid <= -100,-100,per_mid)
    ),
...
top_data <- top_prop_results %>% 
  ...
  filter(variable %in% c("mean_eez"))


top_data_ready_plot=data.frame()

for(i in c(1:nrow(top_data))){
  tmp=data_for_connection(top_data$start_lon[i], top_data$start_lat[i], top_data$end_lon[i], top_data$end_lat[i] , i)
  tmp$per_early_plot=top_data$per_early_plot[i]
  tmp$per_mid_plot=top_data$per_mid_plot[i]
  tmp$source=top_data$source[i]
  tmp$objective=top_data$target[i]
  top_data_ready_plot=rbind(top_data_ready_plot,tmp)
}

top_data_ready_plot <- top_data_ready_plot %>% 
  mutate(
    per_early = ifelse(per_early_plot >= 50,50,per_early_plot),
    per_mid = ifelse(per_mid_plot >= 50,50,per_mid_plot)) %>% 
  gather("time_step","percentage",per_early,per_mid)
...
top_sf_ready_plot <- top_data_ready_plot %>% 
  filter(time_step == "per_early") %>% 
  st_as_sf(coords = c("lon", "lat")) %>% 
  st_set_crs(4326) %>% # to match shapefiles
  st_transform(crs = "+proj=eck4") 

# Get arrow data
top_arrows_data <- as.data.frame(st_coordinates(st_cast(top_sf_ready_plot,"POINT")$geometry)) %>% 
  bind_cols(top_data_ready_plot %>% 
              filter(time_step == "per_early")
  )


# Get point data
top_point_data <- top_data %>% 
  gather("time_step","percentage",per_early:per_mid) %>% 
  filter(time_step == "per_early",
         percentage <= 0) %>% 
  st_as_sf(
...
top_ssr_map <-
  ...
  #         size = 0.1,
  #         col = "white") +
  geom_sf(data = subset(World_map,sovereignt != "Antarctica"),
          size = 0.1, fill = "grey90", col = "gray30") +
  geom_sf(data = subset(top_sf_ready_plot, percentage >= 0), aes(color = percentage), size = 0.1, fill = "white") +
  geom_sf(data = top_point_data, aes(), color = "grey90") +
  geom_path(data = subset(top_arrows_data, percentage >= 0),
            aes(X, Y,
                group = group,
                col = percentage
            ),
  ...
  scale_color_gradientn(name= "Changes in catch proportion and changing stocks (%)",
                        colours = pal,
                        limits = c(0,max(top_sf_ready_plot$percentage,na.rm=T)),
                        breaks = seq(0,max(top_sf_ready_plot$percentage,na.rm=T),10),
                        labels = c(seq(0,max(top_sf_ready_plot$percentage-10,na.rm=T),10),">50")
  ) +
  scale_fill_gradientn(name= "Changes in catch proportion and changing stocks (%)",
                       colours = pal,
                       limits = c(0,max(top_sf_ready_plot$percentage,na.rm=T)),
...
### Noise for all ensemble members


```{r read_raw_data, eval = T, echo = F}
...
sens_paths <- list.files(my_path("R","Emergence_alt_sens"),full.names = T)

# Load all sp in one table
Eme_Alt_Res <- bind_rows(lapply(sens_paths, fread2)) %>% 
  mutate(method= "alt") %>% 
  left_join(Neighbours,
            by = c("eez_name","eez_neighbour")
  ) %>% 
  ...
  clean_names() %>% 
  mutate(tresh = str_to_lower(tresh)) %>% 
  filter(emerging_yr != 2100)


# Combine both datas
sens_data <- Eme_Res %>% 
  filter(taxon_key %in% Eme_Alt_Res$taxon_key) %>% 
  mutate(method = "org") %>% 
  bind_rows(Eme_Alt_Res)
...
n_eez <- length(unique(sens_data$sf_eez_name))

```

#### General numbers

```{r sens_numbers, eval = T, echo = F}

# Total number of emerging stocks

# Global estimates
sens_data %>% 
  group_by(tresh,neighbour_id,method) %>% 
  summarise(
    emerging_stocks = length(unique(taxon_key))
  ) %>% 
  group_by(tresh,method) %>% 
  summarise(
    emerging_stocks = sum(emerging_stocks),
    prop_total = (emerging_stocks/n_stock)*100,
  )

# Looks like there is no difference in the total number of emerging stocks
# tresh     method emerging_stocks prop_total
# 1 tresh_one alt               6665      116. 
# 2 tresh_one org               5729      100  
# 3 tresh_two alt               5258       91.8
# 4 tresh_two org               4108       71.7
...
# Looks like the alternative method sets a later mean ToE but no real difference in s.d. min nor max
#  tresh     method  mean    sd   min   max
# 1 tresh_one alt    2055.  28.7  2007  2099
# 2 tresh_one org    2029.  26.7  2006  2099
# 3 tresh_two alt    2057.  28.6  2007  2099
# 4 tresh_two org    2036.  28.1  2006  2099
...
stock_d <-  
  ...
  # left_join(Neighbours) %>% 
  # distinct(taxon_key,neighbour_id, .keep_all = TRUE) %>% 
  group_by(taxon_key,neighbour_id,tresh,method) %>% 
  summarise(
    taxon_emer = min(emerging_yr)
  ) %>% 
  group_by(taxon_emer,tresh,method) %>% 
  summarise(
    n_tax = n()
  ) %>% 
  group_by(tresh,method) %>% 
...
eez_d <-  sens_data %>% 
  ...
  filter(n_ens == 10) %>% 
  # distinct(eez_name,neighbour_id, .keep_all = TRUE) %>% 
  group_by(sf_eez_name,tresh,method) %>% 
  summarise(
    taxon_emer = min(emerging_yr)
  ) %>% 
  group_by(taxon_emer,tresh,method) %>% 
...
count_eez_plot <- ggplot() +
  ...
  facet_wrap(~ method);count_eez_plot
...
count_stock_plot <- ggplot() +
  ...
  facet_wrap(~ method);count_stock_plot
...
percentage_plot <- ggplot() +
  ...
  facet_wrap(~ method);percentage_plot
...
# Save plot
save_plot(
  "./Figures/cumu_per_freq_sens.png",
  plot_all,
  base_height = 5,
  base_width = 12
)
...
# Per EEZs
Eez_d_hist <- sens_data %>% 
  group_by(eezid,sau_fishing_entity,tresh,method) %>% 
  summarise(
    mean_yr = mean(emerging_yr,na.rm=T),
    sd_yr = sd(emerging_yr,na.rm=T),
    n_taxa = length(unique(taxon_key))
...
# Estimate the value (catch and USD) of emerging species
Eme_value_data <- sens_data %>% 
  group_by(sau_fishing_entity,fishing_entity_id,tresh,taxon_key,method) %>% 
  summarise(n()) %>% 
  left_join(Sau_data,
            by = c("fishing_entity_id","taxon_key")) %>% 
  filter(!is.na(variable)) %>% 
  group_by(sau_fishing_entity,fishing_entity_id,tresh,variable,method) %>% 
  summarise(
    sum_value = sum(value,na.rm=T),
    n_taxa = length(unique(taxon_key)) 
  )
...
fish_d_hist <- sens_data %>% 
  group_by(sau_fishing_entity,tresh,method) %>% 
  summarise(
    value = length(unique(taxon_key))
  ) %>% 
  filter(!is.na(sau_fishing_entity))
...
# Call plot
# -------------------------- #
tresh_list <- "tresh_two"

name_save <- paste("./Figures/emer_map_sens_",tresh_list,".png",sep = "")
...
map <- ggplot(toe_map_data) +
  ...
  scale_fill_viridis("",
                     limits = c(2000,2100),
                     breaks = seq(2000,2100,25),
                     labels = c(">2100",seq(2075,2000,-25)) # not inluding the > because we
  ) +
  my_ggtheme_m(leg_pos = "bottom") +
  facet_wrap(~method,
             nrow = 2)
...
Buble_data <- Eez_d_hist %>% 
  ...
  left_join(Matching_names,
            by = c("sau_fishing_entity","fishing_entity_id")) %>% 
  distinct(sau_fishing_entity,tresh,.keep_all = TRUE) %>%
...
### Function for estimating percentile
...
Percentile_value <- Buble_data %>% 
  group_by(tresh) %>%  # for whole distribution
...
134/length(unique(Buble_data$sau_fishing_entity))
...
Labels_data <- tibble(
  ...
  y = c(-5.2,
        # -5.8,-5.8,
        Percentile_value$x+.1),
  label = c("UN 2030 Agenda", 
            # "Mid shift","Late shift",
            "Fisheries > 75th percentile")
)
...
ggplot(Buble_data) +
  ...
  geom_vline(xintercept = 2030, color = "grey50", size = 0.5, linetype = "dashed") +
  geom_text(data = subset(Labels_data, x == 2029),
            aes(
              x = x,
              y = y,
              label = label
  ...
  geom_text(data = subset(Labels_data, x == 2007),
            aes(
              x = x,
              y = y,
              label = label
...
## Fig. 4N Top fishers on bars

The bar option by UN sub region of Figure 4N of top fishers

```{r}

# Create data
Species_Data <- top_toe %>% 
  filter(tresh == selected_tresh,
         variable == "value") %>% 
...
Species_Data <- Species_Data %>% arrange(group,mean)
...
ggplot(Species_Data, 
  ...
  geom_segment(data=grid_data,
               ...
               colour = "grey90", alpha=1, size=0.3, inherit.aes = FALSE) +
  geom_bar(aes(x=as.factor(id),
               y=mean,
               fill=mean
  ),
  ...
  #           fontface="bold",
  #           inherit.aes = FALSE) +
ylim(-100,100) + # sets the circle the first value the central the last value how wide
  theme_minimal() +
  theme(
  ...
  coord_polar(start = 0) +
  scale_fill_viridis("Average year of range shift\nfor top 5 most valuable stocks",
                     limits = c(0,100),
                     breaks = seq(0,100,20),
                     labels = seq(2000,2100,20),
                     direction = -1
  ) +
  guides(
    fill=guide_colorbar(title.position="top", 
...
ggsave(
  ...
  height = 13,
  units = "in",
  filename = "./Figures/top_stocks_circular_trend.png"
)

```


## Figure 5 code from Juan
...
Data <- Prop_results %>% 
  ...
  # gather("variable","value",per_early:per_early_plot) %>% 
  filter(
    variable %in% c("mean_eez","n_stock")#,
...
network_data_sf <- Data %>%
  ...
  purrr::map(st_linestring) %>%
  # st_sfc(crs = 4326) %>%
  st_sfc(crs = "+proj=eck4") %>% # for tubular plot
  st_sf(geometry = .) %>%
  bind_cols(Data) %>%
  select(everything(), geometry) %>% 
  # Set circles ('cus the world is not flat!)
  # st_segmentize(units::set_units(100, km)) %>%
  # mutate(geometry = (geometry + c(18000000,90000000)) %% c(18000000) - c(18000000,90000000)) %>%
  # mutate(geometry = (geometry + c(180,90)) %% c(360) - c(180,90)) %>%
  # st_wrap_dateline(options = "WRAPDATELINE=YES", quiet = TRUE); network_data_sf # for tubular plot
  st_wrap_dateline(options = c("WRAPDATELINE=YES",  "DATELINEOFFSET=60"), quiet = TRUE) 
...
# Paper numbers for Rev. 1

n_taxa_data %>% 
  group_by(time_step) %>% 
  summarise_at(vars(avrg_spp_per),
               c(mean,sd,min,max),
               na.rm=T)
...
Prop_map <- ggplot() +
  ...
  geom_point(data = subset(Map_data, variable =="mean_eez" & time_step == time & per_plot < 0), 
             aes(start_lon,
                 start_lat
             ),
             size = 0.05,
             ...
             shape = 20,
             stroke = 1.5) +
  geom_path(data = subset(paths, variable =="mean_eez" & time_step == time & per_plot > 0), # filter data 
            aes(X,
                Y,
                group = interaction(line_id,subline_id),
                col = per_plot
  ...
  my_ggtheme_m(leg_pos = "bottom") +
  scale_colour_viridis(
    name= "Changes in catch proportion \nand changing stocks (%)",
...
# Legend
map_legend <- get_legend(Prop_map)
...
plot_all <- ggdraw() +
  ...
  # Europe
  draw_plot(Eu_map, x = 0.53, y = 0.05, width = 0.35, height = 0.35) +
  # Legend
  draw_plot(map_legend, x = 0.25, y = 0, width = 0.59, height = 0.08) +
  # Labels
