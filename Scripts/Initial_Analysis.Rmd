...
plot_data <- stock_d %>% 
  ...
  filter(tresh == "Tresh_one") %>% 
  mutate(
    my_facet = ifelse(metric == "per","Percentage (%)","Cumulative sum"),
    label = ifelse(total == 478,"Total number of stocks","Total number of EEZs")
  );plot_data
...
Index_data <- Eez_d_hist %>% 
  ...
  group_by(sau_fishing_entity,variable) %>% 
  summarise(
...
Value_map_data <- Index_data %>% 
  select(sau_fishing_entity,fishing_entity_id,tresh,variable,participation = index)
...
Average_data <- Dem_pel %>% 
  group_by(un_sub_region) %>% 
  summarise(mean_toe = mean(emerging_yr,na.rm=T)) %>% 
  mutate(regional_average="regional average")
  

# Dem Pelagic boxplot
ggplot(Dem_pel) +
  geom_hline(data = Average_data,
             aes(
               yintercept = mean_toe,
               colour = regional_average
             ),
             linetype = "dashed"
             ) +
  geom_boxplot(
    aes(
      x = reorder_within(DemersPelag,emerging_yr,un_sub_region),
      y = emerging_yr,
      fill = DemersPelag
    ),
    alpha = 0.8
  ) +
  facet_wrap(~un_sub_region,scales = "free_x") +
