...
```{r fun_SummaryProp, eval = T, echo =F}
  ...
  SummaryProp <- function(taxon_key){
...
EEZProp <- function(taxon_list){

 spp <- bind_rows(
   lapply(taxon_list,SummaryProp)
   )
 
 eez_summary <- spp %>% 
   group_by(eez_name,time_step) %>% 
   summarise(
      min_spp = min(mean_spp),
     max_spp = max(mean_spp),
...
taxon_list <- c(600015,600017)
taxons <- c(600004,600004,600015,600017)
x <- NULL
x <- PropChange(taxons)
...
PropChange <- function(taxon_list){

 eez <- EEZProp(taxon_list)
 
 proportion_change <- eez %>% 
   gather("variable","value",min_spp:sd_spp) %>% 
   spread(time_step,value) %>% 
   mutate_at(vars(
     "per_mid" = mid,
     "per_end" = n_end),
...
taxon_list <- gsub("\\..*","",species_list)


Restuls <- PropChange(taxon_list)
