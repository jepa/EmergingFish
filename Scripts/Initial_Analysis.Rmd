...
Buble_data <- Eez_d_hist %>% 
  ...
  mutate(
    ...
    sum_value_bi = sum_value/1000000000) %>%   # Billion usd
  left_join(Matching_names,
            by = c("sau_fishing_entity","fishing_entity_id")) %>% 
...
Labels_data <- tibble(
  x = c(2020,2040,2007),
  y = c(3.2,3.2,Percentile_value$x+.1),
...
ggplot(Buble_data) +
  ...
  scale_y_continuous("Fishing revenue (billion USD)",
                     trans = "log10",
                     # limits = c(3,12),
                     # breaks = seq(3,12,3),
                     expand = c(0,0)
