...
# Species depth classification (SAU-fishbase)
...
##### Fig. 3 Mapping the time of emergence
...
Note this chunk needs the previous one to be ran up to the for-loop
...
#   height = 6,
#   units = "in",
#   filename = "./Figures/eez_map_supp.png"
# )
...
### Figure S6. Regional plots

Note that the following chunk needs the previous one to be ran
...
Lat_am_eez <- World_eez_sf %>% 
  ...
  left_join(Matching_names) %>% 
  filter(
    un_sub_region == "Latin America and the Caribbean",
...
Lat_am_data <- sf_ready_plot %>% 
  filter(source %in% Lat_am_eez$sf_eez_name,
         objective %in% Lat_am_eez$sf_eez_name)

Lat_am_paths <- arrows_data %>% 
  filter(source %in% Lat_am_eez$sf_eez_name,
         objective %in% Lat_am_eez$sf_eez_name)

Lat_am_points <- point_data %>% 
  filter(source %in% Lat_am_eez$sf_eez_name,
         target %in% Lat_am_eez$sf_eez_name)

Lat_am_n_taxa <- n_taxa_map %>% 
  filter(
...
lat_am_map <- ggplot() +
  ...
  geom_sf(data = subset(Lat_am_n_taxa, sovereignt != "Antarctica"),
          aes(fill = avrg_spp_per),
          size = 0.1,
          col = "white") +
  # geom_sf(data = grey_land, size = 0.1, fill = "grey50", col = "gray30") +
  geom_sf(data = subset(Lat_am_data, percentage >= 0), aes(color = percentage), size = 0.1, fill = "white") +
  geom_sf(data = Lat_am_points, aes(), color = "grey90") +
  geom_path(data = subset(Lat_am_paths, percentage >= 0),
            aes(X, Y,
                group = group,
                col = percentage
            ),
...
# Read proportion results
suppressMessages(
  top_prop_results <- PropChange(top_taxon_list, links="y") %>%
    filter(!is.na(eez_neighbour)) %>%
    mutate(per_mid_plot =ifelse(per_mid >= 100, 100,
                                ifelse(per_mid <= -100,-100,per_mid)
    ),
...
#  unpack the coordinates of each feature 
coord_data <- as.data.frame( st_coordinates(st_cast(top_network_data_sf,"MULTILINESTRING")$geometry))

c(top_network_data_sf$line_id, top_network_data_sf$coords) %<-% (
  coord_data %>% 
    rename(subline_id = L1) %>%   
    nest(-L2)
)
...
# Proportion map data 
top_map_data <- top_data %>% 
  gather("time_step","per_chng",per_early:per_mid) %>% 
  mutate(per_plot = ifelse(per_chng > 50,50,per_chng))

time <- "per_early"

World_eez_sf <- World_eez_sf %>% 
  sf::st_transform(crs = "+proj=eck4")
...
top_rop_map <- ggplot() +
  geom_sf(data = World_eez_sf, size = 0.1, fill = "white", col = "gray30") +
  geom_sf(data = subset(World_map,sovereignt != "Antarctica"), size = 0.1, fill = "grey50", col = "gray30") + # created for figure 2
  
  geom_point(data = subset(top_map_data, variable =="mean_eez" & time_step == time & per_plot < 0), 
             aes(start_lon,
                 start_lat
             ),
             size = 0.05,
             ...
             shape = 20,
             stroke = 1.5) +
  geom_path(data = subset(top_paths, variable =="mean_eez" & time_step == time & per_plot > 0), # filter data 
            aes(X,
                Y,
                group = interaction(line_id,subline_id),
                col = per_plot
