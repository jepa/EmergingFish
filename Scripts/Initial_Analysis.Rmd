...
# ------------ #
  ...
  path_name <- paste(my_path("R",extra_path = "Proportion_n"),name,sep= "")
...
TransIndex <- function(Centroids, # Expecting the results from the GetCentroids function
  ...
  ggplot() +
    ...
    theme_classic() +
    ggsave(paste(taxon_key,Id,"plot_example.jpg",sep="_"),
       plot = last_plot(),
       width = 10,
       height = 8,
       units = "in",
  ...
  # Identifies the year of emergence (e.g. trend > noise)
  Final_Result <- mean_sd_index %>%
    filter(year > Hist_y,
...
GetTransIndex <- function(taxon_key,
  ...
  print("Starting Step One")
    ...
    name <- paste(my_path("R",extra_path = "/Emergence/"),taxon_key,"_emergence.txt",sep="")
...
SummaryProp <- function(taxon_key,links="NA"){
  ...
  # Reads taxon data
  proportion_data <- fread(paste(my_path("R",extra_path = "Proportion/"),"proportion_",taxon_key,".csv", sep=""))
...
# Get taxon list
Taxon_List <- list.files(paste(my_path("R"),"Proportion",sep=""))
...
x <- ggplot() +
  ...
  scale_colour_viridis(
    ...
    limits = c(0,max(Data$per_mid)),
    breaks = seq(0,max(Data$per_mid),5),
    # option="magma",
    direction = 1
  ) +
  ggsave("figure_connections_n50.png",
         width = 10,
         height = 6,
         units = "in"
  )
...
# Get taxon list
Taxon_List <- list.files(my_path("R",extra_path = "Emergence_yr"))

# clean names for function
Taxon_List <- gsub("_.*","",Taxon_List) 

Paths <- paste(my_path("R",extra_path = "Emergence_yr"),Taxon_List,"_emergence.txt",sep="")

# Load all sp in noe table
...
hist_data <- stock_d_hist %>% 
  bind_rows(eez_d_hist)

ggplot() +
  geom_density(data = subset(hist_data, level == "EEZ"),
    aes(
      x = emerging_yr,
      fill = level
    ),
  ...
  geom_density(data = subset(hist_data, level == "stock"),
    aes(
      x = emerging_yr,
      fill = level
    ),
...
plot_data <- stock_d %>% 
  ...
  geom_text(aes(label="Total number of Taxa", x = 2010, y = length(unique(Eme_Res$taxon_key))+10), color = "grey70",size =3) +
    ...
    my_ggtheme_p() +
    scale_fill_viridis(discrete = T) +
  ggsave("./Figures/figure_accumulative_curve_B.png",
         width = 6,
         height = 6,
         units = "in"
  )
