...
TransIndex <- function(Centroids, # Expecting the results from the GetCentroids function
  ...
  # Future results
  future <- Centroids_id %>% # FIX ME CUS I PRODUCE A BUNCH OF NAs WITH SSP 600076 ENS 102 AND 104
    filter(year > Hist_y)
  ...
  Partial <- Trans_index %>% 
    ...
    # Filter out ensembels that have no data
    filter(!is.na(value)) %>% 
...
write_csv(Emerging_result,
          paste(my_path("R"),"Emerging_Results.csv",sep="")
)
