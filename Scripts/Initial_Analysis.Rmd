...
x <- top_toe %>% 
  filter(mean_yr <= 2021,
         variable == "value",
         tresh == selected_tresh)

length(unique(x$sau_fishing_entity))/length(unique(Sau_data$fishing_entity_id))
...
mean(Top_map_data$mean)
sd(Top_map_data$mean)

World_map_top <- World_map %>%
...
# These are regions without transboundary species
grey_land <- World_map %>% 
  filter(!sau_fishing_entity %in% Top_map_data$sau_fishing_entity) %>% 
  filter(!sovereignt %in% c("Antarctica"))
