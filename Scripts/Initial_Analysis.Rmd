...
SummaryProp <- function(taxon_key,links="NA"){
  ...
  proportion_tresh <- proportion_data %>% 
    ...
    mutate(
      top_tresh = ensemble_mean+ensemble_sd,
      low_tresh = ensemble_mean-ensemble_sd
    ) %>% 
...
**Note: Contains step to filter out changes less than +-1 s.d.**
...
EEZProp <- function(taxon_list,links="NA"){
  ...
  if(links == "y"){
    ...
    # Min, max and median change per eez_name and neighbour
    eez_summary <- spp %>% 
...
PropChange <- function(taxon_list,links="NA"){
  ...
  proportion_change <- eez %>% 
    gather("variable","value",min_eez:n_per) %>% 
...
# --------------- #
# Get taxon list
Taxon_list <- list.files(my_path("R",extra_path = "Proportion_2005"))
...
```{r Time_lapse_data,  eval = T, echo = T}
...
cont_eez <- plot_data %>% 
  ...
  select(-1) %>% 
  mutate(
    value = ifelse(metric == "per",100,232),
    taxon_emer = c(seq(2082,2099,1),seq(2082,2099,1))
  )
...
# Percentage increase of EEZ after 2020
lm_eez <- plot_data %>% 
  filter(taxon_emer <= 2020,
         tresh == "Tresh_one",
         metric == "per")

summary(lm(lm_eez$taxon_emer~lm_eez$value))
...
plot <- ggplot() +
  ...
  # geom_line(data = subset(plot_data, metric == "com_sum"),
  #           aes(
  #             x = as.numeric(taxon_emer),
  #             y = total,
  #             fill = level
  ...
  #           colour="grey30",
  # )  +
  # geom_text(data = subset(plot_data, metric == "com_sum"),
  #           aes(
  #             x = 2037,
  #             y = total+5,
  #             label = label
  ...
  facet_wrap(~my_facet,
             scales = "free") + 
  # scale_y_continuous("Cumulative number",expand = expand_scale(mult = c(0, .05))
  #                    ) +
  scale_x_continuous("Time of emergence",
  ...
  ggsave("./Figures/cumu_per_freq.png",
  width = 12,
  height = 7,
  units = "in"
  )
...
Index_data <- Eez_d_hist %>% 
  left_join(Value_map_data) %>% 
  filter(
    tresh == "Tresh_one",
    metric == "mean_yr"
  ) %>% 
  group_by(sau_fishing_entity,fishing_entity_id,variable) %>% 
  summarise(
    toe = mean(value,na.rm=T),
    # sum_value = sum(sum_value),
    mean_part = mean(participation,na.rm=T)
  ) %>% 
  filter(!is.na(variable)) %>% 
  # spread(variable,sum_value) %>% 
  spread(variable,mean_part) %>% 
  mutate(
    imp = catch+value,
    index = imp/toe
  )
...
# Get min and maximums
min_index <- min(Index_data$index)
max_index <- max(Index_data$index)

Index_data <- Index_data %>% 
  mutate(
    index_norm =(index-min_index)/(max_index-min_index),
    index_2000 = 2000 + (index-min_index)*(2100-2000)/(max_index-min_index)
  ) 
...
Value_map_data <- Index_data %>%
  select(sau_fishing_entity,fishing_entity_id,
         # tresh,
         # variable,
         participation = index_2000)
...
grey_land <- World_map %>% 
  ...
  filter(!sovereignt %in% c("Antarctica","Brazil"))

grey_eez <- World_eez_sf %>% 
  filter(!eezid %in% Eez_d_hist$eezid,
         !eez_name %in% Neighbours$eez_name)

Brazil_land <- World_map %>% 
  filter(sovereignt == "Brazil") 

Brazil_eez <- World_eez_sf %>% 
  filter(eez_name %in% c("Brazil","USA (Alaska, Arctic)"))
...
Eez_hist <- Eme_Res %>% 
  ...
  map <- World_eez_sf %>% 
    ...
    geom_sf(data = Brazil_eez, aes(), fill = "lightblue", color =  "white",size = 0.1) +
    geom_sf(data = Brazil_land, aes(), fill = "lightblue", color =  "white",size = 0.1) +
    # Set Participation labels
    annotate("text",
             x =-135,
             y = -69,
             label = "Vulnerabillity index",
             color="black",
             size=4,
             angle=0,
             fontface="bold",
  ...
  boxplot <- Eme_Res %>% 
    ...
    filter(tresh == "Tresh_one",
           !is.na(un_sub_region)) %>% 
    mutate(un_sub_region = ifelse(str_detect(un_sub_region,"Latin"),"Ltn. Ame. and the Car.",paste(un_sub_region))) %>% 
    ggplot() +
    geom_boxplot(
      aes(
        # x = un_sub_region,
    ...
    facet_wrap(~un_geo_region,
               scales = "free_x",
               nrow = 1) +
    scale_fill_viridis("UN Sub region",
                       discrete = T) +
    scale_color_grey("") +
    xlab("") +
    ...
    my_ggtheme_p() +
    theme(
      axis.text.x = element_text(size = 10, angle = 45, face = "plain", hjust = 1), 
  ...
  p <- ggdraw() +
    ...
    draw_plot(map, x = 0, y = 0.4, width = 0.95, height = 0.6) +
    # Regional boxplot
...
## Top fishing nations
Buble_data %>% 
  filter(log_value > Percentile_value$x) %>% 
  View()
...
Labels_data <- tibble(
  x = c(2020,2040,2007),
  y = c(-5.8,-5.8,Percentile_value$x+.1),
  label = c("Early emergence", "Late emergence","Fisheries > 75th percentile")
)
...
ggplot(Buble_data) +
  ...
  geom_label_repel(data = subset(Buble_data, sum_value_bi >= 1),
                   aes(
                     x = mean_yr,
                     y = log_value,
...
Eme_Res %>% 
  ...
  # scale_fill_viridis("Habitat preference",
                     # discrete = T) +
  xlab("") +
  ylab("Time of emergence") +
  my_ggtheme_p() +
...
n_taxa_data <- Prop_results %>% 
  ...
  left_join(Matching_names) %>% 
  group_by(sau_fishing_entity,fishing_entity_id) %>% 
  summarise(avrg_spp_per = mean(early,na.rm=T))
...
mean(n_taxa_data$avrg_spp_per,na.rm=T)
...
plot_all <- ggdraw() +
  ...
  draw_plot(Lat_am_map, x = 0.01, y = 0.02, width = 0.33, height = 0.45) +
  # Europe
  draw_plot(Eu_map, x = 0.33, y = 0.01, width = 0.33, height = 0.45) +
  # The Med
  draw_plot(Med_map, x = 0.66, y = 0.02 , width = 0.33, height = 0.45) +
  # Legend
  draw_plot(map_plot, x = 0.3, y = 0, width = 0.59, height = 0.1) +
  # Labels
  draw_plot_label(label = c("A", "B", "The Caribbean","North West Europe","The Mediterranean"),
                  size = c(20,20,15,15,15),
                  x = c(0.01,0.01,0.058,0.38,0.70),
                  y = c(1,0.45,0.44,0.44,0.44)
  )
...
M1 <- lm(emerging_yr ~ sst_toe + bot_toe + status + biome + dem_pel + DemersPelag, data = Main_data)

M1_sum <- summary(M1)
M1_aic <- AIC(M1)
M1_r <-  M1_sum$adj.r.squared

# Model 2; non demPel
M2 <- lm(emerging_yr ~ sst_toe + bot_toe + status + biome + dem_pel, data = Main_data)
...
# Model 3; non biome
M3 <- lm(emerging_yr ~ sst_toe + bot_toe + status + dem_pel + DemersPelag, data = Main_data)
...
# Model 4; non status
M4 <- lm(emerging_yr ~ sst_toe + bot_toe + biome + dem_pel + DemersPelag, data = Main_data)
...
# Model 5; non sst
M5 <- lm(emerging_yr ~ status + bot_toe + biome + dem_pel + DemersPelag, data = Main_data)
...
# Model 6; all + basin
M6 <- lm(emerging_yr ~ sst_toe + bot_toe + status + biome + dem_pel + DemersPelag, data = Main_data)
...
M7 <- lm(emerging_yr ~ sst_toe + bot_toe + category + biome + dem_pel + DemersPelag, data = Main_data)
...
M8 <- lm(emerging_yr ~ sst_toe + bot_toe + category + as.factor(biome) + DemersPelag + latitude, data = Main_data)

M8_sum <- summary(M8)
M8_aic <- AIC(M8)
M8_r <-  M8_sum$adj.r.squared

# Model 9; Include UN regions
M9 <- lm(emerging_yr ~ sst_toe + bot_toe + category + biome + DemersPelag + latitude + un_region, data = Main_data)

M9_sum <- summary(M9)
M9_aic <- AIC(M9)
M9_r <-  M9_sum$adj.r.squared



tibble(
  "model" = paste("M",seq(1,9,1),sep=""),
  "AIC" = c(M1_aic,M2_aic,M3_aic,M4_aic,M5_aic,M6_aic,M7_aic,M8_aic, M9_aic),
  "rsquared" = c(M1_r,M2_r,M3_r,M4_r,M5_r,M6_r,M7_r,M8_r,M9_r)
)

# The Winner is M9    49283.   0.0765
...
M9_sum %>% 
  ...
  # stargazer(title = "Multiple regression results for time of emergence of Transboundary stocks. Only showing explanatory variables. The model significantly predicts time of emergencec (F(31,5270) = 15.38, p < 0.001, R2 = 0.06).",
            # align=TRUE)
  # knitr::kable() %>% 
  write_csv(.,
            "./Tables/mlr_result.csv")

#Table for supplements
M9_sum %>% 
  tidy() %>% 
  # filter(p.value < 0.05) %>% 
  # mutate(p.value = ifelse(p.value < 0.001,0.001,round(p.value,3))) %>% 
  # stargazer(title = "Multiple regression results for time of emergence of Transboundary stocks. Only showing explanatory variables. The model significantly predicts time of emergencec (F(18,6155) = 20.68, p < 0.001, R2 = 0.05).",
            # align=TRUE)
  # knitr::kable() %>% 
  write_csv(.,
            "./Tables/mlr_result_sup.csv")
```
