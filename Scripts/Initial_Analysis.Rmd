...
TransIndex <- function(Centroids, # Expecting the results from the GetCentroids function
  ...
  return(Final_Result)
    ...
    Final_Result <- mean_sd_index %>%
      ...
      # summarise(emerging_yr= min(year)) %>% # First year that treshold is crossed
...
```{r Time_lapse_plot, eval = T, echo = F}
...
count_eez_plot <- ggplot() +
  ...
  scale_y_continuous("Cumulative number of EEZs (n)",
                     expand = c(0,0)
                     ) +
  scale_x_continuous("",
...
count_stock_plot <- ggplot() +
  ...
  scale_y_continuous("Cumulative number of stocks (n)",
                     expand = c(0,0)
                     ) +
  scale_x_continuous("Time of emergence of transboundary index",
                     breaks = seq(2000,2100,25),
                     limits = c(2005,2100),
                     # label = c(seq(2000,2075,25),">2100"),
                     expand = c(0, 0)
...
# Draw figure
  plot_all <- ggdraw() +
    ...
    draw_plot(count_eez_plot, x = 0.01, y = 0, width = 0.28, height = 1) +
    # Europe
    draw_plot(count_stock_plot, x = 0.28, y = 0, width = 0.28, height = 1) +
    # Percentage plot
    draw_plot(percentage_plot, x = 0.56, y = 0 , width = 0.28, height = 1) +
    # legend
    draw_plot(legend, x = 0.84, y = 0 , width = 0.28, height = 1)
...
#   
  ...
  map <- World_eez_sf %>% 
    ...
    my_ggtheme_m() +
    theme(
      legend.box.margin = margin(t = -80, l = 40),
...
ggplot(Buble_data) +
  ...
  # right side labels
  geom_label_repel(data = subset(Buble_data, mean_yr > 2035 & sum_value_bi >= 1),
                  aes(
                    x = mean_yr,
                    y = log_value,
                  ...
                  box.padding = 0.2,
                  fill = "NA",
                  nudge_x = 2050 + subset(Buble_data, mean_yr > 2035 & sum_value_bi >= 1)$mean_yr,
                  segment.size  = 0.2,
  ...
  geom_label_repel(data = subset(Buble_data, mean_yr < 2035 & sum_value_bi >= 1),
                  ...
                  box.padding = 0.2,
                  fill = "NA",
                  nudge_x = 2010 - subset(Buble_data, mean_yr < 2035 & sum_value_bi >= 1)$mean_yr,
                  segment.size  = 0.2,
                  segment.color = "grey50",
                  direction     = "y",
                  hjust         = 1
...
sd(n_taxa_data$avrg_spp_per,na.rm=T)
...
for(i in 1:2){
  ...
  plot_all <- ggdraw() +
    ...
    # Labels
    draw_plot_label(label = c("A", "B", "The Caribbean","Northwestern Europe","The Mediterranean"),
                    size = c(20,20,15,15,15),
                    x = c(0.01,0.01,0.058,0.33,0.68),
                    y = c(1,0.45,0.44,0.44,0.44)
    )
  
  
  name <- paste0("./Figures/prop_",time,".png")
...
```{r data_loading, eval = T, echo = F}

# Get taxon list
Taxon_list_sens <- list.files(my_path("R",extra_path = "Toe_Sens_per/Emergence_2005_p20"))

# clean names for function
Taxon_list_sens <- gsub("_.*","",Taxon_list_sens) 
