...
# Per EEZs
Eez_d_hist <- Eme_Res %>% 
  group_by(eezid= eez_id ,sau_fishing_entity,tresh) %>% 
  # summarise(n_tax = length(unique(taxon_key))) %>% 
  summarise(
...
ggplot(subset(eez_d_hist, tresh == "Tresh_one")) +
  geom_density(
    aes(
      x = value,
      fill = tresh
...
for(i in 1:2){
  ...
  name_save <- paste("./Figures/emer_map_par_",tresh_list[i],".png",sep = "")
  
  World_map_nemer <- World_map_nemer %>% 
    filter(
      tresh ==  tresh_list[i], 
      variable == "value"
    )
