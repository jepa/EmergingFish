...
# Done with the help of https://www.data-to-viz.com/story/MapConnection.html
...
p <-
  ...
  my_ggtheme_m(leg_pos = "bottom") +
    geom_path(data = subset(arrows_data, percentage >= 0),
              ...
              show.legend = NA) +
  my_ggtheme_m(leg_pos = "bottom") +
  scale_color_gradientn(name= "Changes in catch proportion \nand changing stocks (%)",
                        colours = pal,
  ...
  scale_fill_gradientn(name= "Changes in catch proportion \nand changing stocks (%)",
                        colours = pal,
                        limits = c(0,max(sf_ready_plot$percentage,na.rm=T)),
                        breaks = seq(0,max(sf_ready_plot$percentage,na.rm=T),10),
                        labels = c(seq(0,max(sf_ready_plot$percentage-10,na.rm=T),10),">50")
...
## ------------------- ##
  ...
  Lat_am_map <- ggplot() +
    ...
    geom_sf(data = grey_land, size = 0.1, fill = "grey50", col = "gray30") +
    geom_sf(data = subset(n_taxa_map, sovereignt != "Antarctica" & time_step == time_toplot),
            aes(fill = avrg_spp_per),
            size = 0.1,col = "white") +
