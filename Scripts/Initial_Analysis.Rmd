...
network_data_sf <- Data %>%
  ...
  # mutate(geometry = (geometry + c(18000000,9000000)) %% c(360) - c(18000000,9000000)) %>%
...
Prop_map <- ggplot() +
  geom_sf(data = subset(World_map, sovereignt != "Antarctica"), size = 0.1, fill = "gray50", col = "gray50") +
...
Lat_am_map <- ggplot() +
  ...
  geom_sf(data = Lat_am_land, size = 0.1, fill = "gray50", col = "gray40") +
  geom_point(data = subset(Lat_am_data, time_step == time & per_chng < 0 ),
             aes(start_lon,
                 start_lat,
                 col = per_chng),
             size = 1,
             ...
             shape = 20,
             stroke = 1.5) +
  geom_path(data = subset(Lat_am_paths, time_step == time & per_chng > 0),
            aes(X, Y,
                group = interaction(line_id,subline_id),
                col = per_chng
            ),
  ...
  scale_colour_viridis(
    name="Average Gain of\nStock-Share Ratio (%)",
    limits = c(0,max(Lat_am_data$per_chng)),
    breaks = seq(0,max(Lat_am_data$per_chng),5),
    # option="magma",
    # direction = -1
  );Lat_am_map
...
Med_map <- ggplot() +
  ...
  geom_sf(data = Med_land, size = 0.5, fill = "gray50", col = "gray40") +
  geom_point(data = subset(Med_data, time_step == time & per_chng < 0 ),
             aes(start_lon,
                 start_lat,
                 col = per_chng),
             size = 1,
             ...
             shape = 20,
             stroke = 1.5) +
  geom_path(data = subset(Med_paths, time_step == time & per_chng > 0),
            aes(X, Y,
                group = interaction(line_id,subline_id),
                col = per_chng
            ),
  ...
  scale_colour_viridis(
    name="Average Gain of\nStock-Share Ratio (%)",
    limits = c(0,max(Med_data$per_chng)),
    breaks = seq(0,max(Med_data$per_chng),5),
    # option="magma",
    # direction = -1
  );Med_map
...
Eu_map <- ggplot() +
  ...
  geom_sf(data = Eu_land, size = 0.5, fill = "gray50", col = "gray40") +
  geom_point(data = subset(Med_data, time_step == time & per_chng < 0 ),
             aes(start_lon,
                 start_lat,
                 col = per_chng),
             ...
             shape = 20,
             stroke = 1.5) +
  geom_path(data = subset(Eu_paths, time_step == time & per_chng > 0),
            aes(X, Y,
                group = interaction(line_id,subline_id),
                col = per_chng
            ),
  ...
  scale_colour_viridis(
    name="Average Gain of\nStock-Share Ratio (%)",
    limits = c(0,max(Eu_data$per_chng)),
    breaks = seq(0,max(Eu_data$per_chng),5),
    # option="magma",
    # direction = -1
  );Eu_map
...
# Per EEZs
eez_d_hist <- Eme_Res %>% 
  group_by(eezid  = sf_eezid,fishing_entity,tresh) %>% 
  # summarise(n_tax = length(unique(taxon_key))) %>% 
  summarise(
...
World_map_nemer <- World_map %>%
  mutate(fishing_entity = ifelse(fishing_entity == "United States","USA", fishing_entity)) %>% 
  left_join(fish_d_hist,
            by = "fishing_entity")

for(i in 1:2){
  
  Tresh_list <- c("Tresh_one","Tresh_two")
  name_save <- paste("./Figures/emer_map_",Tresh_list[i],".png")
  ...
  map <- World_eez_sf %>% 
    ...
    geom_sf(data = World_map_nemer, aes(fill = value +2000   # to match scales
                                        ),
      color =  "white",
      size = 0.1
      ) +
