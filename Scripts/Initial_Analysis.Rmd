...
```{r fun_EEZexit, eval = F, echo = F}

# taxon_list <- c(600004,600006)
...
# EEZProp <- function(taxon_list,links="NA"){
  ...
  spp <- bind_rows(
    ...
    # mutate_if(is.numeric,replace_na,0) %>% # If NA 0 meaning wins and looses
    mutate_at(vars(
      "per_early" = early,
      "per_mid" = mid
    ),
  ...
  gather("time_step","percentage_change",per_early:per_mid)
