...
data_ready_plot <- data_ready_plot %>% 
  mutate(
    per_early_plot = ifelse(per_early_plot >= 50,50,per_early_plot),
    per_mid_plot = ifelse(per_mid_plot >= 50,50,per_mid_plot))

sf_ready_plot <- st_as_sf(data_ready_plot,
            coords = c("lon", "lat")
            ) %>% 
  st_set_crs(4326) %>% # to match shapefiles
  st_transform(crs = "+proj=eck4")
...
p <-
  ...
  geom_sf(data = subset(sf_ready_plot, per_early_plot >= 0), aes(color = per_early_plot), size = 0.1, fill = "white") +
  geom_sf(data = point_data, aes(), color = "grey90") +
  my_ggtheme_m(leg_pos = "bottom") +
    geom_path(data = subset(arrows_data, per_early_plot >= 0),
              aes(X, Y,
                  group = group,
                  col = per_early_plot
              ),
  ...
  scale_color_gradientn(name= "Changes in catch proportion \nand changing stocks (%)",
                        colours = pal,
                        limits = c(0,max(data_ready_plot$per_early_plot,na.rm=T)),
                        breaks = seq(0,max(data_ready_plot$per_early_plot,na.rm=T),10),
                        labels = c(seq(0,max(data_ready_plot$per_early_plot-10,na.rm=T),10),">50")
  ) +
