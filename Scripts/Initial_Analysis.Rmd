...
Buble_data <- Eez_d_hist %>% 
  ...
  filter(tresh == tresh_list[i],
         variable == "value") %>% 
  mutate(sum_value_mi = sum_value/1000000) %>%   # Million usd
  left_join(Matching_names,
            by = c("sau_fishing_entity","fishing_entity_id")) %>% 
  ...
  ungroup() %>% 
  mutate(
    sau_fishing_entity = ifelse(sum_value_mi >= 1000 & !sau_fishing_entity %in% c("Spain"), paste(sau_fishing_entity,"*",sep = ""),sau_fishing_entity)
  )
...
ggplot(Buble_data) +
  ...
  # scale_size(range = c(.1, 10), name="Fishing revenue (USD)")  +
  geom_label_repel(data = subset(Buble_data, sum_value_mi >= 1000),
                   aes(
                     x = mean_yr,
                     y = sum_value_mi,
  ...
  scale_color_viridis(name="Georaphic Region",
                      discrete = T) +
  scale_x_continuous("Average time of emergence",
                     breaks = seq(2000,2080,20)
  ) +
  scale_y_continuous("Fishing revenue (million)",
