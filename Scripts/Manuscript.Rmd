...
Our results suggest that `r n_eez_per`% (n = `r n_eez`) of the world EEZs will see a change in SSR by early $21^{st}$ century (Fig. 1). The change in SSR will range from gains of over 100% of a sertain shared stock (e.g. Guatemala' SSR of *species* shared with Mexico is projected to go from 12% to 26%) to loses of up to -71% of SSR (e.g. Honduras SSR of *species* shared with the Nicaragua in the Pacific coast is projected to decline from 50% to 14%). Despite no substantial changes in SSR per EEZ can be seen by the mid $21^st$ century (**Supplements Fig 1 (map)**), changes in SSR are expected to increase in intensity (**Supplements Fig 2 (histogram)**).
