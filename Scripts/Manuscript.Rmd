...
We estimated changes in the Stock Share Ratio of transboundary species shared by neighboring EEZs by early (2020-2040) and mid $21^{st}$ century (2040-2060), relative to present (1951-2005). Our results suggest that `r n_eez_per`% (n = `r n_eez`) of the world EEZs will see a change in SSR by 2030 (Fig. 1A). While most EEZs will experience an average change > 20%, a few EEZs are expected to have larger changes in SSR (Fig. 1A). For example, Guatemala' average SSR of *species* shared with Mexico is projected to go from 12% to 26% while Honduras SSR of *species* shared with the Nicaragua in the Pacific coast is projected to decline from 50% to 14%. The number of stocks per EEZ that will see a change in SSR by early $21^{st}$ century is relatively low (*XX%*), however, some regions like West Africa and the Mediterranean will see changes in over 50 species. These regions, where EEZs are relatively small and have multiple borders will see a complex dynamic of changes in SSR with no clear patterns (Fig. 2B). Towards the mid $21^st$ century, changes in SSR per EEZ will no be substantial (**Supplements Fig 1 (map)**) relative to early century, but are expected to increase in intensity (**Supplements Fig 2 (histogram)**).
...
We estimated the time of emergence between neighboring EEZs for 633 transboundary stocks. Since 2006, 87% of the world EEZs have already seen at least one emerging stock and 60% of the transboundary species analyzed have emerged in at least one neighboring EEZ (e.g. Peruvean anchoveta emerged in 2010 between Chile and Peru). The cumulative sum of emerging stocks will have a steeper increase until 2040 where the increment will be slightly less intense. Similarly, for the number of EEZs, the increment curve seem to flatten by 2020. By 2081, all of the EEZs analyzed here will have at least one emerging stock while 2097 will be the year where all 633 stocks have emerged in at least one neighboring EEZ (Fig. 2). 


```{r SSR_gain_map, eval = F,  echo = F, fig.width=9, fig.height=12, fig.cap =  "Cumulative increase in time of emergence by EEZ and stock"}
...
On average, most of the world EEZ's will see a stock ToE by 2030 with a higher frequency of number of emerging stocks between latitudes 0$^o$ and 50$^o$N (Fig 2). In most cases, the current participation of the emerging stocks in a country's total fishing revenue is less than 50%, however, in some cases it represents 85% of total fishing revenue (Peru) or more, like the case of island territories such as Marshall Islands (92%) or Tokelau- New Zealand (100%). In few cases like Brazil's EEZ and Alaska's polar EEZ, no stock will emerge.
...
The regional patterns of ToE suggest that most stocks will emerge by early 21st with many stocks in the Americas and Oceania already emerging. For the larger fisheries of the world in terms of revenue representing 75 percentile, only the USA will see an average ToE before 2030 while fisheries in all other top fishing countries will be emerging within the following 10 years.
...
The mean ToE of transboundary species varied significantly according to the geographic region of the neighboring EEZs (Kruskal-Wallis, $X^2$ = 287.23, *DF* = 93, *p* < 0.001) (Fig. 5). Post-hoc tests revealed that EEZs in Easter Asia who have the latest average ToE across all habitat types (2041), and Latin America and the Caribbean and Polynesia who have the earliest average ToE across all habitats (2021 and 2018, respectively), significantly differ with almost all other regions. Other regions also significantly differ from each other but in a less frequent manner (*See Table Sup X for full statistics*). When looking specifically at habitat association, we also found significant differences on ToE (Kruskal-Wallis, $X^2$ = 286.48, *DF* = 93, *p* < 0.001). Specifically, the ToE of species with preferences for pelagic-oceanic and reef habitats is significantly different than other for other habitat preferences (*See Table Sup XX for full statistics*).
...
We hypothesize that species environmental preferences (e.g. presence in certain biomes and habitat preference), climate change effects on water temperature (e.g. ToE of SST), human drivers (e.g. catch rate), and latitudinal gradient would have significant influence in the ToE of transboundary stocks (*Table Sup 3*). Results from the multiple linear regression analysis show a significant relation between ToE of transboundary stocks and latitudinal gradient (*p* > 0.001), tropical (*p* = 0.013) and temperate (*p* = 0.002) biomes, EEZs in the Americas (*p* > 0.001), Asia (*p* > 0.001) and Oceania (*p* = 0.002), and some habitats (Table 1).

Table 2. Multiple regression results for time of emergence of transboundary stocks. Only showing significant variables. The model significantly predicts time of emergence (F[18,6155] = 20.68, p < 0.001, R2 = 0.05).
