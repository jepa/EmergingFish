...
author: 'Authors: Juliano Palacios-Abrantes*^1^, Thomas Frölicher^2^, U. Rashid Sumaila^1^,  Vicky
  Lam^1^, David VanderZwaag^3^, and William W.L. Cheung^1^'
...
The world is moving towards sustainable development and ocean-based activities plays a mayor role in achieving international policies.
...
Climate change is expected to reduce the production from global fisheries in about XX% to xx% [@Cheung; @Barange] compromising our capacity to reach SDG 14.

Recent research show that the impacts of climate change on the productivity of marine wild fisheries can be offset with the proper reform of current fisheries management [@Gaines; @CheungClimateReadyFish]. While bringing fisheries that are poorly managed to a sustainable level will likely offset the negative impacts of climate change to fisheries production [@Gaines], it will not necessarily solve the spatial consequences of species moving from one management area to another.

Shifts in species distribution does not necessarily mean a reduction in production (e.g. reduction in stock biomass) but rather a spatial re-distribution of the stock's biomass [@RefLiam?].

Documented cases of stock shifts have resulted in international conflict and sustainable fishing rates by parties involved in the conflict.

This is of specific concern to a set of 633 fished species that cross international borders, also known as transponder species [@FishForVisa].

With the implementation of arbitrary spatial management boundaries, at least 633 species became transboundary representing an average catch (2005 - 2010) of 47.6 million tons and generating USD 65,483 million in fishing revenue [@FishorVisa].


Transboundary fish stocks refer to those species that cross international jurisdictions known as Economic Exclusive Zones (EEZs) of neighboring nations [@FAO]. These stocks are responsible for an average catch (2005 - 2010) of 47.6 million tons generating USD 65,483 million in fishing revenue [@FishorVisa].
...
We tested data for normallity (e.g. skewness, kurtosis) and performed a couple of non-parametric Kruskal–Wallis test by ranks [@Hollander:2013wz] to investigate geopolitical and ecological differences in the time of emergence of transboundary stocks. Moreover, we implemented a multiple linear regression analysis to a series to explain the ToE of each species in each EEZ. Specifically, we lookd at habitat preferences, catch trends, geopolitical and biome location and ToE of both bottom and sea surface temperature (See *Databases and species selection*). We hypothesize that all of these factors would have significant influence in the time of emergence of transboundary stocks. All analyses were run using the statistical software *R version 3.5.2 (2018-12-20; Eggshell Igloo)* with the packages *data.table* [@Packagedatatable:2019uh], *janitor* [@PackagejanitorSim:2018ur], *rfishbase* [@PackagerfishbaseR:2019th], *sf* [@PackagesfSimpleF:2018vp], *sp* [@PackagespClasses:2019tn], *tidiverse* [@PackagetidyverseE:2017vq], *tidytext* [@PackagetidytextTe:2019vk], *gmt* [@gmt] and *zoo* [@PackagezooSInfr:2019tm], *readxl* [@REF], *ggrepel* [@REF], *zeallot* [@REF], *viridis* [@], *cowplot* [@], *moments* [@REF] and *pgirmess* [@REF]. All code is available at https://github.com/jepa/TransEmergence
...
We estimated changes in the Stock Share Ratio of transboundary species shared by neighboring EEZs by early (2020-2040) and the mid $21^{st}$ century (2040-2060), relative to present (1951-2005). Our results suggest that `r n_eez_per`% (n = `r n_eez`) of the world EEZs will see a change in SSR by 2030 (Fig. 1A). While most EEZs will experience an average change lower than 25%, a few EEZs are expected to have larger changes in SSR relative to early century. Such change is expected to slightly increase in intensity towards the  mid $21^{st}$ century (**Supplements Fig 1 (histogram)**). For example, Guatemala's SSR of *Panulirus gracilis* shared with Mexico is projected to go from 12% to 26%. The number of stocks per EEZ that will see a change in SSR by early $21^{st}$ century is relatively low (median of 13 spp per EEZ), however, some regions like West Africa and the Mediterranean will see changes in over 50 species (e.g. Senegal, 94). These regions, where EEZs are relatively small and have multiple borders will see a complex dynamic of changes in SSR with no clear patterns (Fig. 1B). Towards the mid $21^st$ century, changes in SSR per EEZ will no be substantial (**Supplements Fig 1 (map)**) relative to early century, but are expected to increase in intensity (**Supplements Fig 1 (histogram)**).
...
![Average gain of transboundary stock share ratio (lines) and number of shifting stocks (polygon) by 2030 (2021-2040) relative to 1951-2005. Arrows go from the losing EEZ (point) to the winner (arrowhead). Pannel B zooms in to specific areas ](../Figures/Fig1.png)

## Time of Emergence

We estimated the ToE between neighboring EEZs for 633 transboundary stocks, that is, the first year where the future stock's distrtibution overpasses the historical variation. Since 2006, 87% of the world EEZs have already seen at least one emerging stock and 60% of the transboundary species analyzed have emerged in at least a set of neighboring EEZ (e.g. *Engraulis ringens* shared by Chile and Peru emerged in 2010). The cumulative sum of emerging stocks will have a steeper increase until 2040 where the increment will be slightly less intense (Fig. 2). Similarly, for the number of EEZs, the increment curve seem to flatten by 2020. By 2081, all of the EEZs analyzed here will have at least one emerging stock while 2097 will be the year where all 633 stocks have emerged in at least one neighboring EEZ (Fig. 2). 
...
![Cumulative increase in time of emergence by EEZ and stock](../Figures/Fig2.png)

On average, most of the world EEZ's will see a stock emerge by 2030 with a few EEZs having a relative late average ToE (e.g. Honduras average ToE is 2071) and 54 EEZs having already emergered (e.g. average ToE >= 2020). A higher number of emerging stocks will happen between latitudes 0$^o$ and 50$^o$N corresponding to regions of relatively small EEZs with multiple borders (e.g. The Carribeean and the Medditerranean) (Fig 2). In most cases, the current participation of the emerging stocks in a country's total fishing revenue is less than 50%. However, in some cases emerging stocks represent 85% of total fishing revenue (e.g. Peru) or more (e.g. Marshall Islands, 92%). Only in very few cases like Brazil and Alaska's polar EEZ, presented no emerging stock between 2006 and 2100. 
...
![(Average time of emergence (ToE) per Exclusive Economic Zone (EEZ polygon) and participation of emerging stocks in total EEZ fishing revenue (land polygon). Darker tones represent low participation and late ToE; lighter colors represent large participation and earlier ToE. Density plot on right shows the number of emerging stocks across EEZ centroids (10^o running mean))](../Figures/Fig3.png)

Most of the main fishing countries in terms of revenue from transbounudary fisheries (e.g. the top $75^{th}$ percentile) will have an average ToE across all species after 2030 (n = 28). The remaining twelve countries will have an average ToE before 2030, this includes the United States who is the $4^{th}$ place it total reveneue (Fig. 4). These 47 countries are also among those with the largest number of emerging taxa. Regionally speaking, most of African, Asian and European EEZs will see a latter ToE, while the Americas and Oceania will see an early ToE (Fig. 4). From the top ten countries with more revenue from transboundary species, nine of them share a border. Most of these countries (e.g. Spain and China) will also have a large number of emering stocks, however, some others like Peru and Norway will have a small number of emerging taxa that represents high revenue fisheries.
...
![(Average time of emergence (ToE) per fishing entity. Fishing revenue in logarithmic sacle. Showing names for the top 75th percentile). Countries marked with a asterisk have at least one border](../Figures/Fig4.png)

We compared species ToE within EEZs by geogrpahic region and by species habitat preference. The mean ToE of transboundary species varied significantly according to the geographic region of the neighboring EEZs (Kruskal-Wallis, $X^2$ = 287.23, *DF* = 93, *p* < 0.001) (Fig. 5). Post-hoc tests revealed that EEZs in Easter Asia who have the latest average ToE across all habitat types (2041), and Latin America and the Caribbean and Polynesia who have the earliest average ToE across all habitats (2021 and 2018, respectively), significantly differ with almost all other regions. Other regions also significantly differ from each other but in a less frequent manner (*See Table Sup X for full statistics*). When looking specifically at habitat association, we found significant differences on ToE of different habitat types (Kruskal-Wallis, $X^2$ = 286.48, *DF* = 93, *p* < 0.001). Specifically, the ToE of species with preferences for pelagic-oceanic and reef habitats was significantly different than other for other habitat preferences (*See Table Sup XX for full statistics*).
