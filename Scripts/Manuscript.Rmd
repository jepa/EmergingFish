...
Knowing the time (e.g. year) in whitch SSR is going to diverge from a historical variablillty can be key for policy to prepare to such change. In here we call this event time of emergence and defined it as the moment in time when the future variation in SSR becomes larger than the historical trend variabillity [@Roggers:2015jq]. We emplot the ten GFDL ensable members described above, and developed an index of transboundary range shift, fisheries and climate change (*I*). The index is based on the variation of a species distribution centroid in realtion to the EEZs it ocuppys as follows:

$$I =(\frac{D_{A,y}}{sd(D_{A,h})} - \frac{D_{B,y}}{sd(D_{B,h)}})^2$$

where $D_A$ and $D_B$ represent the mean (among ensamble members) distribution centroid of a species in EEZs A and B, respectiveley for each future time step from 2006 to 2100 (*t*) and *sd* is the standar deviation of the historical (*h*, 1951 - 2005) centroid distribution for $D_A$ and $D_B$.
...
This way, for two neighbouring EEZs sharing a stock, if the centroid shifts towards EEZ A ($D_A$ decreases) but away from EEZ A B ($D_B$ increases), EEZ A may catch a bigger share over B and the risk of dispute increases. Consequently, if the centroid shifts are within historical variability, risk of dispute between EEZ A and B is lower, finally, if the centroid shifts are beyond historical variability, the risk of dispute is higher. Thus, risk is positively related to the frequency of centroid shifts beyond historical variability (Fig. 1). 
...
prop_path <- "/Volumes/HALL2000/Data/TransEmergence/Results/Proportion_2005/"
prop_names  <- list.files(prop_path)

prop_read <- paste(prop_path,prop_names,sep="")

results <- bind_rows(lapply(prop_read, data.table::fread))
...
n_eez_per <- n_eez/275*100


### Max values
max_mid <- prop_r %>% 
  filter(variable == "max_spp" & 
           !is.na(per_mid)
         )
...
Our results suggest that `r n_eez_per`% (n = `r n_eez`) of the world EEZs will see a change in SSR by mid $21^{st}$ century (Fig. 1). The change in SSR will range from gains of over 100% (e.g. Honduras' SSR of *species* shared with Mexico is projected to go from 28% to 60%) to loses of up to -80% of SSR (e.g. Nauru' SSR of *species* shared with the Marshall Isl. is projected to decline from 14% to 3%). By the end of the century, SSR changes will be more exrteme with 13 countries gaining over 100% SSR and Palau loosing -93.61% of the SSR of *spp* to the	Philippines (from 20% to 1.2%).
...
img <- readPNG("../Figures/figure_connections_n50.png")
grid.raster(img)

```


![Average transboundary species SSR gain of neighbouring EEZs by 2030 (2021-2040) relative to 1951-2005 under RCP 8.5. Arrows go from the loosing EEZ (point) to the winner (arrowhead)](../Figures/figure_connections_n50.png)
